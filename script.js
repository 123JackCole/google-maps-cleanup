...
(async () => {
  ...
  const scrollContainer = document.querySelector('[role="main"] div[tabindex="-1"]');
  ...
  console.log("✅ Load complete. Starting removal...");
  const results = { success: [], failed: [], notFound: [] };

  for (const name of namesToRemove) {
    // 1. Find all restaurant containers on the page
    const containers = Array.from(document.querySelectorAll('.m6QErb.XiKgde'));
    
    // 2. Find the specific container where the headline matches our name
    const targetContainer = containers.find(el => {
      const headline = el.querySelector('.fontHeadlineSmall');
      return headline && headline.innerText.trim() === name;
    });
    ...
    // 3. Find the Delete button inside that specific container
    const deleteBtn = targetContainer.querySelector('button[aria-label="Delete"]');
    ...
    if (deleteBtn) {
      ...
      /* 
         NOTE: If a "Are you sure you want to delete?" popup appears, 
         uncomment the two lines below:
      */
      // const confirmBtn = document.querySelector('button[jsaction*="confirm"]');
      // if (confirmBtn) confirmBtn.click();
      
    } else {
      console.warn(`❌ Found container for "${name}", but no Delete button.`);
      results.failed.push(name);
    }
  }

  console.table({
    "Total Successfully Removed": results.success.length,
    "Failed (Button missing)": results.failed.length,
    "Not Found in List": results.notFound.length
  });
})();